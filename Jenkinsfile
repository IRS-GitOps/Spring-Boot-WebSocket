  ...
  stages('Sonar Scan & Maven Compile/Deploy to Nexus')
  {
    ...
    stage('Deploy to Nexus'){
     ...
     stage('Trigger Release Candidate') {
       steps {
         cloudBeesFlowTriggerRelease configuration: 'Thunder-CD', parameters: '{"release":{"releaseName":"tj-Spring-Boot-WebSocket", "stages":[{"stageName":"Release Readiness","stageValue":true},{"stageName":"Pre-Prod","stageValue":true},{"stageName":"Prod","stageValue":true}]}}', projectName: 'tjohnson Demo', releaseName: 'tj-Spring-Boot-WebSocket', startingStage: 'Release Readiness'
       }
     }
  }  
}
